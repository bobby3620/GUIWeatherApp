...
public class OpenWeatherMapAPI extends APITools{
    ...
    public OpenWeatherMapAPI(String location, String API_KEY){
        ...
        // Makes the first value an uppercase and every letter after a space an uppercase
        changeStateLetters(this.location);
        // This gets the country's name for the app
        try {
            getCountryName();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private void changeStateLetters(String location){

        this.location = location.substring(0,1).toUpperCase() + location.substring(1);

        for(int i = 1; (i < location.length() - 1); i++){
            if(location.charAt(i) == ' '){
                this.location = location.substring(0,1).toUpperCase() + location.substring(1, i+1) + location.substring(i+1, i+2).toUpperCase() + location.substring(i+2);
            }
        }
    }

    private void getCountryName() throws IOException{
        ...
        parseGetCountryName(response.toString());
    }

    private void parseGetCountryName(String country){
        if(country.contains("country")){
            int indexOfCountry = country.indexOf("country");
            int indexOfTimezone = country.indexOf("timezone");
            // When we add 8, it'll start at the first letter of the country
            // When we subtract 11, it'll start at the last letter of the country and get any middle values
            // Works because each XML call is the exact same -> start at country/>, end at <
            this.country = country.substring(indexOfCountry + 8, indexOfTimezone - 11);
        }
    }
    ...
    // Opens a connection for the forecasted weather at a given location
    ...
    // This method receives the information from response StringBuilder, then is found in the JSON object
    // and sets the fwList to the values needed/wanted for the x(7 in this case) amount of days we want the forecasted weather
